...
def _ui_asset_sequence(prefix, count):
    paths = []
    for index in range(1, count + 1):
        paths.append(config.ui_path("%s_%02d.png" % (prefix, index)))
    return tuple(paths)
...
def _switch_map_direct_fallback(target_map_id, target_record, spawn_x=None, spawn_y=None):
    ...
    _release_map_boss_sheet_if_not_current()
...
def _map_boss_for_map(map_id=None):
    if map_id is None:
        map_id = current_map_id
    return MAP_BOSS_DEFINITIONS.get(map_id)


def _map_boss_for_enemy_id(enemy_id):
    if not enemy_id:
        return None
    for boss in MAP_BOSS_DEFINITIONS.values():
        if boss.get("enemy_id") == enemy_id:
            return boss
    return None
...
def _ensure_map_boss_sheet_loaded(boss=None):
    ...
    if (
        ...
        print("map_boss_sheet_api_missing")
        return False
    ...
    if ok:
        ...
        map_boss_sheet_loaded_map_id = boss_map_id
...
def _release_map_boss_sheet():
    global map_boss_sheet_loaded_map_id, map6_boss_sheet_loaded
    if map_boss_sheet_loaded_map_id is None and not map6_boss_sheet_loaded:
        return
    if hasattr(lgfx, "enemy_sheet_clear"):
        lgfx.enemy_sheet_clear()
    map_boss_sheet_loaded_map_id = None
    map6_boss_sheet_loaded = False
    print("map_boss_sheet_released")


def _release_map_boss_sheet_if_not_current():
    if map_boss_sheet_loaded_map_id is not None and map_boss_sheet_loaded_map_id != current_map_id:
        _release_map_boss_sheet()


def _ensure_map6_boss_sheet_loaded():
    return _ensure_map_boss_sheet_loaded(MAP_BOSS_DEFINITIONS.get(MAP6_ID))


def _release_map6_boss_sheet():
    _release_map_boss_sheet()
...
MAP9_FOREST_BOSS_BATTLE_SPRITE_PATH = config.ui_path("map9_forest_boss_battle.png")
MAP9_FOREST_BOSS_SHEET_PATH = config.sprite_path("map9_forest_boss_sheet.rgb565")
MAP10_ICE_BOSS_BATTLE_SPRITE_PATH = config.ui_path("map10_ice_boss_battle.png")
MAP10_ICE_BOSS_SHEET_PATH = config.sprite_path("map10_ice_boss_sheet.rgb565")
MAP11_FIRE_BOSS_BATTLE_SPRITE_PATH = config.ui_path("map11_fire_boss_battle.png")
MAP11_FIRE_BOSS_SHEET_PATH = config.sprite_path("map11_fire_boss_sheet.rgb565")
MAP6_BOSS_SHEET_W = 576
MAP6_BOSS_SHEET_H = 384
...
MAP6_BOSS_ANIM_SEQUENCE = (0, 1, 2, 3, 4, 5)
MAP_BOSS_SHEET_W = MAP6_BOSS_SHEET_W
...
MAP_BOSS_DEFINITIONS = {
    MAP6_ID: {
        "map_id": MAP6_ID,
        "enemy_id": "MAP6_BOSS",
        "sheet_path": MAP6_BOSS_SHEET_PATH,
        "sheet_w": MAP6_BOSS_SHEET_W,
        "sheet_h": MAP6_BOSS_SHEET_H,
        "frame_w": MAP6_BOSS_FRAME_W,
        "frame_h": MAP6_BOSS_FRAME_H,
        "center_x": MAP6_BOSS_CENTER_X,
        "center_y": MAP6_BOSS_CENTER_Y,
        "trigger_radius": MAP6_BOSS_TRIGGER_RADIUS_PX,
        "anim_frame_ms": MAP6_BOSS_ANIM_FRAME_MS,
        "anim_sequence": MAP6_BOSS_ANIM_SEQUENCE,
        "lock_portals_until_defeated": False,
    },
    MAP9_ID: {
        "map_id": MAP9_ID,
        ...
        "anim_sequence": MAP_BOSS_ANIM_SEQUENCE,
        "lock_portals_until_defeated": True,
    },
    MAP10_ID: {
        "map_id": MAP10_ID,
...
MAP3_MIMISTITCH_OPENING1_PNG = config.ui_path("map3_mimistitch_opening_01.png")
MAP3_MIMISTITCH_OPENING2_PNG = config.ui_path("map3_mimistitch_opening_02.png")
MAP3_MIMISTITCH_OPENING3_PNG = config.ui_path("map3_mimistitch_opening_03.png")
MAP3_MIMISTITCH_ACT_PRAISE_PNG = config.ui_path("map3_mimistitch_act_praise.png")
MAP3_MIMISTITCH_ACT_TIDY_PNG = config.ui_path("map3_mimistitch_act_tidy.png")
MAP3_MIMISTITCH_ACT_ACCEPT_PNG = config.ui_path("map3_mimistitch_act_accept.png")
MAP3_MIMISTITCH_PRAISE1_PNG = config.ui_path("map3_mimistitch_praise_01.png")
MAP3_MIMISTITCH_PRAISE2_PNG = config.ui_path("map3_mimistitch_praise_02.png")
MAP3_MIMISTITCH_PRAISE3_PNG = config.ui_path("map3_mimistitch_praise_03.png")
MAP3_MIMISTITCH_TIDY1_PNG = config.ui_path("map3_mimistitch_tidy_01.png")
MAP3_MIMISTITCH_TIDY2_PNG = config.ui_path("map3_mimistitch_tidy_02.png")
MAP3_MIMISTITCH_ACCEPT1_PNG = config.ui_path("map3_mimistitch_accept_01.png")
MAP3_MIMISTITCH_ACCEPT2_PNG = config.ui_path("map3_mimistitch_accept_02.png")
MAP3_MIMISTITCH_ACCEPT3_PNG = config.ui_path("map3_mimistitch_accept_03.png")
...
MAP6_CRYSTALGOLEM_ACT_OBSERVE_PNG = config.ui_path("map6_crystalgolem_act_observe.png")
MAP6_CRYSTALGOLEM_ACT_REMEMBER_PNG = config.ui_path("map6_crystalgolem_act_remember.png")
MAP6_CRYSTALGOLEM_ACT_TELL_PNG = config.ui_path("map6_crystalgolem_act_tell.png")
MAP6_CRYSTALGOLEM_ACT_TOUCH_CORE_PNG = config.ui_path("map6_crystalgolem_act_touch_core.png")
MAP6_CRYSTALGOLEM_OPENING_PNGS = _ui_asset_sequence("map6_crystalgolem_opening", 10)
MAP6_CRYSTALGOLEM_OBSERVE_PNGS = _ui_asset_sequence("map6_crystalgolem_observe", 2)
MAP6_CRYSTALGOLEM_REMEMBER_PNGS = _ui_asset_sequence("map6_crystalgolem_remember", 8)
MAP6_CRYSTALGOLEM_TELL_PNGS = _ui_asset_sequence("map6_crystalgolem_tell", 6)
MAP6_CRYSTALGOLEM_TOUCH_CORE_PNGS = _ui_asset_sequence("map6_crystalgolem_touch_core", 8)
...
ITEM_DETAIL_ICON_SRC_W = 32
ITEM_DETAIL_ICON_SRC_H = 32
ITEM_DETAIL_ICON_BY_PICKUP_ID = {
    "wood_right_rack_knife": config.ui_path("item_icon_weapon_knife.png"),
    "wood_right_rack_sword": config.ui_path("item_icon_weapon_sword.png"),
    "map3_red_potion": config.ui_path("item_icon_item_red_potion.png"),
    "map3_chest_armor": config.ui_path("item_icon_armor_map3.png"),
    "map4_great_sword": config.ui_path("item_icon_weapon_great_sword.png"),
    "map4_small_red_potion": config.ui_path("item_icon_item_small_red_potion.png"),
    "map4_key": config.ui_path("item_icon_item_map5_door_key.png"),
}
ITEM_DETAIL_ICON_BY_ITEM_ID = {
    "weapon_knife": config.ui_path("item_icon_weapon_knife.png"),
    "weapon_sword": config.ui_path("item_icon_weapon_sword.png"),
    "item_red_potion": config.ui_path("item_icon_item_red_potion.png"),
    "armor_map3": config.ui_path("item_icon_armor_map3.png"),
...
BATTLE_SLOT_ENEMY_MAIN = 0
BATTLE_SLOT_DIALOG_CURRENT = 1
BATTLE_SLOT_ACT_0 = 2
BATTLE_SLOT_ACT_1 = 3
BATTLE_SLOT_ACT_2 = 4
BATTLE_SLOT_ACT_3 = 13
BATTLE_SLOT_MERCY_LOCKED = 5
BATTLE_SLOT_MERCY_SUCCESS = 6
BATTLE_SLOT_FLOWEY_NORMAL = 7
...
LAMP_DIALOG_MS = 2000
WOOD_UP_DIALOG_MS = 3000
ITEM_REPLY_MS = 1000
WOOD_UP_DIALOG_TEXT_W = 214
WOOD_UP_DIALOG_TEXT_H = 27
WOOD_UP_DIALOG_FRAME_PAD_X = 6
WOOD_UP_DIALOG_FRAME_PAD_Y = 4
WOOD_UP_DIALOG_TRIGGER_RADIUS_PAD = 14
WOOD_UP_DIALOG_TRIGGER_TOUCH_PAD = 12
...
SCRIPTED_BATTLE_DIALOG_DELAY_MS = 1500
SCRIPTED_BATTLE_DIALOG_PNG_W = 200
SCRIPTED_BATTLE_DIALOG_PNG_H = 72
SCRIPTED_BATTLE_SIDE_DIALOG_GAP_PX = 10
SCRIPTED_BATTLE_KIND_NONE = 0
SCRIPTED_BATTLE_KIND_OPENING = 1
SCRIPTED_BATTLE_KIND_ACT = 2
SCRIPTED_BATTLE_KIND_MERCY = 3
...
def _make_scripted_dialog_entries(paths, lines, png_w=SCRIPTED_BATTLE_DIALOG_PNG_W, png_h=SCRIPTED_BATTLE_DIALOG_PNG_H):
    entries = []
...
def _make_act_option_entry(path, width, text):
    return {"png": path, "png_w": width, "png_h": 18, "text": text}


MAP6_CRYSTALGOLEM_OPENING_LINES = (
    "停止前進。",
    "孩子，不該進入更深的地方。",
    "判定結果：禁止通行。",
    "弱小的靈魂，會被遺跡吞沒。",
...
MAP6_CRYSTALGOLEM_OBSERVE_LINES = (
    "觀察無意義。",
    "理解不會改變危險。",
)
MAP6_CRYSTALGOLEM_REMEMBER_LINES = (
    "你想起了之前遇見的怪物。",
    "你想起 Gloombell 害怕被丟下。",
...
MAP6_CRYSTALGOLEM_TOUCH_CORE_LINES = (
...
def _map_boss_enemy_profile(enemy_id, display_name, sprite_path):
    return {
        "enemy_id": enemy_id,
...
ENEMY_REGISTRY = {
    ...
    "MAP2_ENEMY1": {
        ...
        "menu_draw_h": NORMAL_BATTLE_ENEMY_DRAW_H,
        "act_options": (
            {"png": MAP2_GLOOMBELL_ACT_WATCH_PNG, "png_w": 46, "png_h": 18, "text": "查看"},
            {"png": MAP2_GLOOMBELL_ACT_CALL_PNG, "png_w": 46, "png_h": 18, "text": "呼喚"},
            {"png": MAP2_GLOOMBELL_ACT_WAIT_PNG, "png_w": 46, "png_h": 18, "text": "等待"},
        ),
        "opening_dialog": (
    ...
    "MAP3_ENEMY1": {
        ...
        "menu_draw_h": NORMAL_BATTLE_ENEMY_DRAW_H,
        "act_options": (
            {"png": MAP3_MIMISTITCH_ACT_PRAISE_PNG, "png_w": 46, "png_h": 18, "text": "稱讚"},
            {"png": MAP3_MIMISTITCH_ACT_TIDY_PNG, "png_w": 46, "png_h": 18, "text": "整理"},
            {"png": MAP3_MIMISTITCH_ACT_ACCEPT_PNG, "png_w": 46, "png_h": 18, "text": "接納"},
        ),
        "opening_dialog": (
            {"png": MAP3_MIMISTITCH_OPENING1_PNG, "png_w": 200, "png_h": 72, "text": "喵……？"},
            {"png": MAP3_MIMISTITCH_OPENING2_PNG, "png_w": 200, "png_h": 72, "text": "你喜歡貓嗎？"},
            {"png": MAP3_MIMISTITCH_OPENING3_PNG, "png_w": 200, "png_h": 72, "text": "你會喜歡……壞掉的貓嗎？"},
        ),
        "act_sequences": (
            (
                {"png": MAP3_MIMISTITCH_PRAISE1_PNG, "png_w": 200, "png_h": 72, "text": "你稱讚 Mimi-Stitch 的耳朵很特別。"},
                {"png": MAP3_MIMISTITCH_PRAISE2_PNG, "png_w": 200, "png_h": 72, "text": "Mimi-Stitch 假裝不在意，但尾巴輕輕晃了一下。"},
                {"png": MAP3_MIMISTITCH_PRAISE3_PNG, "png_w": 200, "png_h": 72, "text": "特別……不是奇怪嗎？"},
            ),
            (
                {"png": MAP3_MIMISTITCH_TIDY1_PNG, "png_w": 200, "png_h": 72, "text": "你假裝幫 Mimi-Stitch 整理縫線。"},
                {"png": MAP3_MIMISTITCH_TIDY2_PNG, "png_w": 200, "png_h": 72, "text": "雖然你沒有真的修好什麼，但牠看起來安心了一點。"},
            ),
            (
                {"png": MAP3_MIMISTITCH_ACCEPT1_PNG, "png_w": 200, "png_h": 72, "text": "你告訴 Mimi-Stitch，不完整也沒有關係。"},
                {"png": MAP3_MIMISTITCH_ACCEPT2_PNG, "png_w": 200, "png_h": 72, "text": "Mimi-Stitch 低下頭，縫線旁的小鈴鐺輕輕響了一聲。"},
                {"png": MAP3_MIMISTITCH_ACCEPT3_PNG, "png_w": 200, "png_h": 72, "text": "那我……可以只當我自己嗎？"},
            ),
        ),
        "act_unlock_order": (0, 1, 2),
        "mercy_success_sequence": (
            {"png": MAP3_MIMISTITCH_MERCY1_PNG, "png_w": 200, "png_h": 72, "text": "我不會再假裝成別人了。"},
            {"png": MAP3_MIMISTITCH_MERCY2_PNG, "png_w": 200, "png_h": 72, "text": "如果你又看到掉在地上的玩具……"},
            {"png": MAP3_MIMISTITCH_MERCY3_PNG, "png_w": 200, "png_h": 72, "text": "可以不要踩到它們嗎？"},
        ),
        "opening_delay_ms": SCRIPTED_BATTLE_DIALOG_DELAY_MS,
        "opening_side_dialog": True,
        "dialog_line_ms": MAP1_STORY_LINE_MS,
    ...
    "MAP6_BOSS": {
        ...
        "menu_draw_h": NORMAL_BATTLE_ENEMY_DRAW_H,
        "act_options": (
            _make_act_option_entry(MAP6_CRYSTALGOLEM_ACT_OBSERVE_PNG, 46, "觀察"),
            _make_act_option_entry(MAP6_CRYSTALGOLEM_ACT_REMEMBER_PNG, 46, "回想"),
            _make_act_option_entry(MAP6_CRYSTALGOLEM_ACT_TELL_PNG, 46, "訴說"),
            _make_act_option_entry(MAP6_CRYSTALGOLEM_ACT_TOUCH_CORE_PNG, 64, "觸碰晶核"),
        ),
        "opening_dialog": _make_scripted_dialog_entries(
            MAP6_CRYSTALGOLEM_OPENING_PNGS,
            MAP6_CRYSTALGOLEM_OPENING_LINES,
            MAP6_CRYSTALGOLEM_OPENING_PNG_W,
            MAP6_CRYSTALGOLEM_OPENING_PNG_H,
        ),
        "act_sequences": (
            _make_scripted_dialog_entries(
                MAP6_CRYSTALGOLEM_OBSERVE_PNGS,
                MAP6_CRYSTALGOLEM_OBSERVE_LINES,
            ),
            _make_scripted_dialog_entries(
                MAP6_CRYSTALGOLEM_REMEMBER_PNGS,
                MAP6_CRYSTALGOLEM_REMEMBER_LINES,
            ),
            _make_scripted_dialog_entries(
                MAP6_CRYSTALGOLEM_TELL_PNGS,
                MAP6_CRYSTALGOLEM_TELL_LINES,
            ),
            _make_scripted_dialog_entries(
                MAP6_CRYSTALGOLEM_TOUCH_CORE_PNGS,
                MAP6_CRYSTALGOLEM_TOUCH_CORE_LINES,
            ),
        ),
        "act_unlock_order": (0, 1, 2, 3),
        "mercy_success_sequence": _make_scripted_dialog_entries(
            MAP6_CRYSTALGOLEM_MERCY_PNGS,
            MAP6_CRYSTALGOLEM_MERCY_LINES,
        ),
        "opening_delay_ms": SCRIPTED_BATTLE_DIALOG_DELAY_MS,
        "opening_side_dialog": True,
        "opening_dialog_desired_w": MAP6_CRYSTALGOLEM_OPENING_PNG_W,
...
class BattleAssetManager:
    def __init__(self):
        self.enemy_id = None
        self.loaded_slots = set()
        self.dialog_slot_loaded = False
        self.context_active = False

    def _has_slot_api(self):
        return (
    ...
    def begin(self, enemy_id):
        self.cleanup()
        self.enemy_id = enemy_id
        self.context_active = True

    def _release_dialog_slot(self):
        if not self._has_slot_api():
            self.dialog_slot_loaded = False
            return
        if not self.dialog_slot_loaded:
            return
        try:
            lgfx.png_slot_release(BATTLE_SLOT_DIALOG_CURRENT)
        except Exception as err:
            print("[battle_assets] dialog_release_fail:", err)
        self.dialog_slot_loaded = False

    def load_slot_once(self, slot_id, path):
        if (not self.context_active) or (not path) or (not self._has_slot_api()):
            return False
        if slot_id in self.loaded_slots:
            return True
        try:
            ok = bool(lgfx.png_slot_load_file(slot_id, path))
        except Exception as err:
            print("[battle_assets] load_fail:", slot_id, path, err)
            ok = False
        if not ok:
            return False
    ...
    def replace_dialog(self, path):
        ...
        if not path:
            return True
        try:
            ok = bool(lgfx.png_slot_load_file(BATTLE_SLOT_DIALOG_CURRENT, path))
        except Exception as err:
            print("[battle_assets] dialog_load_fail:", path, err)
            ok = False
        if not ok:
            self.dialog_slot_loaded = False
            return False
        self.dialog_slot_loaded = True
        return True

    def draw_slot(self, slot_id, x, y, w, h):
        if not self._has_slot_api():
            return False
        if slot_id == BATTLE_SLOT_DIALOG_CURRENT:
            if not self.dialog_slot_loaded:
                return False
        elif slot_id not in self.loaded_slots:
            return False
        try:
            return bool(lgfx.png_slot_draw(slot_id, x, y, w, h))
        except Exception as err:
            print("[battle_assets] draw_fail:", slot_id, err)
            return False

    def cleanup(self):
        self._release_dialog_slot()
        if self._has_slot_api():
            for slot_id in tuple(self.loaded_slots):
                try:
                    lgfx.png_slot_release(slot_id)
                except Exception as err:
                    print("[battle_assets] release_fail:", slot_id, err)
        self.enemy_id = None
...
battle_assets = BattleAssetManager()

current_battle_enemy = ENEMY_REGISTRY.get(DEFAULT_BATTLE_ENEMY_ID)
scripted_battle_active = False
scripted_battle_kind = SCRIPTED_BATTLE_KIND_NONE
scripted_battle_entries = ()
scripted_battle_index = -1
scripted_battle_next_ms = 0
scripted_battle_opening_due_ms = 0
scripted_battle_opening_pending = False
scripted_battle_side_dialog = False
...
map_boss_anim_seq_index = 0
map_boss_anim_last_ms = 0
map_boss_last_draw_frame = -1
map_boss_last_draw_sx = -99999
map_boss_last_draw_sy = -99999
map_boss_scene_composed = False
map_boss_scene_frame = -1
map_boss_scene_sx = -99999
map_boss_scene_sy = -99999
map6_boss_sheet_loaded = False
...
WOOD_UP_INSPECT_OBJECTS = (
    {
        ...
        "detect_radius": 18,
        "hint_radius": 10,
        "png_path": WOOD_UP_BOOKSHELF_DIALOG_PATH,
        "draw_w": 214,
        "draw_h": 27,
    },
    {
        ...
        "detect_radius": 16,
        "hint_radius": 10,
        "png_path": WOOD_UP_MIRROR_DIALOG_PATH,
        "draw_w": 214,
        "draw_h": 27,
    },
    {
        ...
        "detect_radius": 18,
        "hint_radius": 10,
        "png_path": WOOD_UP_BED_DIALOG_PATH,
        "draw_w": 214,
        "draw_h": 27,
    },
)
WOOD_LEFT_INSPECT_OBJECTS = (
    {
        "inspect_id": "bathtub",
        "map_id": WOOD_LEFT_ID,
        "rect": (20, 74, 56, 88),
        ...
        "detect_radius": 20,
        "hint_radius": 10,
        "png_path": WOOD_LEFT_BATHTUB_DIALOG_PATH,
        "draw_w": 214,
        "draw_h": 27,
    },
    {
        ...
        "detect_radius": 16,
        "hint_radius": 10,
        "png_path": WOOD_LEFT_TOILET_DIALOG_PATH,
        "draw_w": 214,
        "draw_h": 27,
    },
    {
        ...
        "detect_radius": 18,
        "hint_radius": 10,
        "png_path": WOOD_LEFT_PLANT_DIALOG_PATH,
        "draw_w": 214,
        "draw_h": 27,
    },
)
...
def _inventory_item_icon_path(item):
    if not item:
        return None

    path = item.get("icon_path")
    if path:
        path = _resolve_runtime_png_path(path)
        if path and _path_exists(path):
            return path

    pickup_id = item.get("origin_pickup_id")
    if pickup_id:
        path = ITEM_DETAIL_ICON_BY_PICKUP_ID.get(pickup_id)
        if path and _path_exists(path):
            return path

    item_id = item.get("id")
    if item_id:
        path = ITEM_DETAIL_ICON_BY_ITEM_ID.get(item_id)
        if path and _path_exists(path):
            return path

    for path in ITEM_ICON_PATHS:
        if path and _path_exists(path):
            return path
    return None


def _inventory_item_stat_text(item):
    if not item:
        return ""
    slot = item.get("equip_slot")
    if slot == "weapon":
        return "AT +%d" % int(item.get("at_bonus", 0))
    if slot == "armor":
        return "DF +%d" % int(item.get("df_bonus", 0))
    if _is_consumable_item(item):
        heal_amount = int(item.get("heal_amount", 0))
        if heal_amount > 0:
            return "HP +%d" % heal_amount
...
def _map_boss_scene_state(loop_start):
    boss = _map_boss_for_map()
    if not _map_boss_should_draw(boss):
        return None
...
def _render_scene(scroll_x, scroll_y, player_x, player_y, force_full, loop_start=None):
    ...
    if loop_start is not None and hasattr(lgfx, "tile_render_player_enemy"):
        boss_state = _map_boss_scene_state(loop_start)
    if boss_state and USE_TILE_RENDER_PLAYER_COMPOSE:
        ...
        map_boss_scene_composed = True
        map_boss_scene_frame = frame_index
        map_boss_scene_sx = boss_sx
        map_boss_scene_sy = boss_sy
...
def _init_runtime_state():
    ...
    global scripted_battle_active, scripted_battle_kind, scripted_battle_entries, scripted_battle_index
    global scripted_battle_next_ms, scripted_battle_opening_due_ms, scripted_battle_opening_pending
    global scripted_battle_side_dialog, scripted_battle_return_to_act, scripted_battle_pending_exit
    ...
    map_boss_battle_enemy_id = None
    map_boss_anim_seq_index = 0
    map_boss_anim_last_ms = time.ticks_ms()
    map_boss_last_draw_frame = -1
    map_boss_last_draw_sx = -99999
    map_boss_last_draw_sy = -99999
    map_boss_scene_composed = False
...
def _update_preload_for_player(px, py):
    ...
    # Release the wood-room dialog PNG before starting a background preload.
    # Map8's first load is especially sensitive to PSRAM peaks, and the dialog
    # slot is otherwise only cleared later in switch_map().
    if _wood_up_dialog_is_active(now):
        _clear_wood_up_dialog()
        explore_overlay_dirty = False
        gc.collect()
...
def _draw_text_left_in_box(x, y, w, h, text, color=BATTLE_COLOR_WHITE):
    if not hasattr(lgfx, "draw_text"):
        return
    clipped = _fit_text_to_width(text, w - 4)
    if not clipped:
        return
    ty = y + ((h - 8) // 2)
    if ty < y:
        ty = y
    lgfx.draw_text(x + 2, ty, clipped, color)
...
def _draw_inventory_item_detail_panel(x, y, w, h, item):
    ...
    _draw_rect_thick(x, y, w, h, BATTLE_COLOR_WHITE, panel_border)
    inner_x = x + 6
    inner_w = w - 12
    if inner_w <= 0:
        return
    if not item:
        _draw_text_left_in_box(inner_x, y + 6, inner_w, 12, "EMPTY", BATTLE_COLOR_WHITE)
        return

    _draw_text_left_in_box(inner_x, y + 6, inner_w, 12, item.get("name", "Item"), BATTLE_COLOR_WHITE)
    _draw_text_left_in_box(inner_x, y + 20, inner_w, 12, _inventory_item_stat_text(item), BATTLE_CMD_COLOR)

    icon_y = y + 38
    icon_h = (y + h - 6) - icon_y
    if icon_h <= 0:
        return
    icon_path = _inventory_item_icon_path(item)
    if icon_path:
        _draw_png_in_box(
            (icon_path, ITEM_DETAIL_ICON_SRC_W, ITEM_DETAIL_ICON_SRC_H),
            inner_x,
            icon_y,
            inner_w,
            icon_h,
...
def _wood_up_dialog_rect():
    draw_w = wood_up_dialog_draw_w if wood_up_dialog_draw_w > 0 else WOOD_UP_DIALOG_TEXT_W
    draw_h = wood_up_dialog_draw_h if wood_up_dialog_draw_h > 0 else WOOD_UP_DIALOG_TEXT_H
    dialog_w = draw_w + (WOOD_UP_DIALOG_FRAME_PAD_X * 2)
    dialog_h = draw_h + (WOOD_UP_DIALOG_FRAME_PAD_Y * 2)
    max_w = ACTIVE_VIEW_W - 8
    max_h = ACTIVE_VIEW_H - 8
    if dialog_w > max_w:
        dialog_w = max_w
    if dialog_h > max_h:
        dialog_h = max_h
...
def _wood_up_dialog_is_active(loop_start):
    return bool(time.ticks_diff(wood_up_dialog_until_ms, loop_start) > 0 and wood_up_dialog_loaded)


def _release_wood_up_dialog_slot():
    global wood_up_dialog_loaded, wood_up_dialog_path
    if wood_up_dialog_loaded and hasattr(lgfx, "png_slot_release"):
        try:
            lgfx.png_slot_release(WOOD_UP_DIALOG_SLOT_ID)
        except Exception as err:
            print("wood_up_dialog_release_fail:", err)
    wood_up_dialog_loaded = False
    wood_up_dialog_path = None
...
def _wood_room_inspect_objects():
    ...
    if current_map_id == WOOD_LEFT_ID:
        return WOOD_LEFT_INSPECT_OBJECTS
    return ()


def _find_wood_up_inspect_target():
    best_obj = None
    best_d2 = 0
    for obj in _wood_room_inspect_objects():
        if obj.get("map_id") != current_map_id:
            continue
        rx = int(obj.get("interact_x", 0))
        ry = int(obj.get("interact_y", 0))
        radius = int(obj.get("detect_radius", 0))
        if radius > 0:
            radius += WOOD_UP_DIALOG_TRIGGER_RADIUS_PAD
            dx = rx - player_x
            dy = ry - player_y
            d2 = (dx * dx) + (dy * dy)
            if d2 <= (radius * radius):
                if (best_obj is None) or (d2 < best_d2):
                    best_obj = obj
                    best_d2 = d2
        rect = obj.get("rect", (0, 0, 0, 0))
        if _player_touches_rect(rect, WOOD_UP_DIALOG_TRIGGER_TOUCH_PAD) or _in_rect(player_x, player_y, rect):
            return obj
    return best_obj
            ...
            hints.append(
                {
                    ...
                    "world_cx": int(rack.get("interact_x", x + (w // 2))),
                    "world_cy": int(rack.get("interact_y", y + h)),
                    "world_r": GROUND_WEAPON_PICKUP_RADIUS,
                }
            )
...
def _interact_hints_paused(loop_start):
    ...
    if time.ticks_diff(lamp_dialog_until_ms, loop_start) > 0:
        return True
    return _wood_up_dialog_is_active(loop_start)
...
def _map_boss_is_defeated(boss):
    if not boss:
        return False
    map_id = boss.get("map_id")
    if map_id == MAP6_ID and map6_boss_defeated:
        return True
    return bool(map_boss_defeated_by_map.get(map_id, False))


def _current_map_boss_blocks_portals():
    boss = _map_boss_for_map()
    return bool(boss and boss.get("lock_portals_until_defeated") and not _map_boss_is_defeated(boss))


def _map_boss_should_draw(boss=None):
    if boss is None:
        boss = _map_boss_for_map()
    if not boss:
        return False
    if boss.get("map_id") != current_map_id:
        return False
    if _map_boss_is_defeated(boss):
        return False
    if not _ensure_map_boss_sheet_loaded(boss):
        return False
...
def _map_boss_draw_rect(boss):
    frame_w = int(boss.get("frame_w", MAP_BOSS_FRAME_W))
    frame_h = int(boss.get("frame_h", MAP_BOSS_FRAME_H))
    half_w = frame_w // 2
    half_h = frame_h // 2
    return (
        int(boss.get("center_x", 0)) - half_w - scroll_x,
        int(boss.get("center_y", 0)) - half_h - scroll_y,
        frame_w,
        frame_h,
    )


def _map_boss_player_overlap(boss):
    bx, by, bw, bh = _map_boss_draw_rect(boss)
    if player_sheet_enabled:
...
def _map_boss_update_anim(boss, loop_start):
    global map_boss_anim_seq_index, map_boss_anim_last_ms
    sequence = boss.get("anim_sequence", MAP_BOSS_ANIM_SEQUENCE)
    if not sequence:
        sequence = MAP_BOSS_ANIM_SEQUENCE
    frame_ms = int(boss.get("anim_frame_ms", MAP_BOSS_ANIM_FRAME_MS))
    if frame_ms < 1:
        frame_ms = MAP_BOSS_ANIM_FRAME_MS
    if map_boss_anim_last_ms == 0:
        map_boss_anim_last_ms = loop_start
    while time.ticks_diff(loop_start, map_boss_anim_last_ms) >= frame_ms:
        map_boss_anim_last_ms = time.ticks_add(map_boss_anim_last_ms, frame_ms)
        map_boss_anim_seq_index += 1
        if map_boss_anim_seq_index >= len(sequence):
            map_boss_anim_seq_index = 0


def _draw_map_boss(loop_start, scene_redrawn=False, player_redrawn=False):
    global map_boss_last_draw_frame, map_boss_last_draw_sx, map_boss_last_draw_sy
    boss = _map_boss_for_map()
    if not _map_boss_should_draw(boss):
        map_boss_last_draw_frame = -1
...
def _map_boss_trigger_enemy_id(px, py):
    boss = _map_boss_for_map()
    if not boss or _map_boss_is_defeated(boss):
        return None
    dx = px - int(boss.get("center_x", 0))
    dy = py - int(boss.get("center_y", 0))
    r = int(boss.get("trigger_radius", MAP_BOSS_TRIGGER_RADIUS_PX))
    if (dx * dx + dy * dy) <= (r * r):
        return boss.get("enemy_id")
    return None
...
def _map_boss_mark_defeated(boss):
    ...
    map_boss_last_draw_frame = -1
    map_boss_anim_seq_index = 0
    if map_boss_sheet_loaded_map_id == map_id:
        _release_map_boss_sheet()
    print("map_boss_defeated:", boss.get("enemy_id"))
    return True


def _map_boss_mark_defeated_for_enemy(enemy_id):
    return _map_boss_mark_defeated(_map_boss_for_enemy_id(enemy_id))


def _map6_boss_should_draw():
    return _map_boss_should_draw(MAP_BOSS_DEFINITIONS.get(MAP6_ID))


def _map6_boss_trigger_hit(px, py):
    return _map_boss_trigger_enemy_id(px, py) == "MAP6_BOSS"


def _map6_boss_mark_defeated():
    return _map_boss_mark_defeated(MAP_BOSS_DEFINITIONS.get(MAP6_ID))
...
def _draw_wood_up_dialog(loop_start):
    if not _wood_up_dialog_is_active(loop_start):
        return False
    dialog_x, dialog_y, dialog_w, dialog_h = _wood_up_dialog_rect()
    _fill_rect_solid(dialog_x, dialog_y, dialog_w, dialog_h, 0x0000)
    _draw_rect_thick(dialog_x, dialog_y, dialog_w, dialog_h, BATTLE_COLOR_WHITE, BATTLE_CMD_BORDER_THICK)
    if hasattr(lgfx, "png_slot_draw"):
        try:
            draw_w = wood_up_dialog_draw_w if wood_up_dialog_draw_w > 0 else WOOD_UP_DIALOG_TEXT_W
            draw_h = wood_up_dialog_draw_h if wood_up_dialog_draw_h > 0 else WOOD_UP_DIALOG_TEXT_H
            draw_x = dialog_x + ((dialog_w - draw_w) // 2)
            draw_y = dialog_y + ((dialog_h - draw_h) // 2)
            return bool(
                lgfx.png_slot_draw(
                    WOOD_UP_DIALOG_SLOT_ID,
                    draw_x,
                    draw_y,
                    draw_w,
                    draw_h,
            ...
            print("wood_up_dialog_draw_fail:", err)
    return False
...
def _clear_battle_frame_interior(frame_x, frame_y, frame_w, frame_h):
    pad = BATTLE_FRAME_BORDER_THICK
    inner_x = frame_x + pad
    inner_y = frame_y + pad
    inner_w = frame_w - (pad * 2)
    inner_h = frame_h - (pad * 2)
    _clear_rect_black(inner_x, inner_y, inner_w, inner_h)
...
def _draw_battle_menu_static_layer(frame_x, frame_y, frame_w, cmd_x0, cmd_y, cmd_w):
    ...
    _clear_battle_frame_interior(frame_x, frame_y, frame_w, BATTLE_FRAME_H)
    ...
    if _map1_story_is_active():
        ...
        if flowey_visible:
            flowey_slot_id = BATTLE_SLOT_FLOWEY_ANGRY if map1_story_enemy_angry else BATTLE_SLOT_FLOWEY_NORMAL
            enemy_drawn = _battle_draw_story_slot(
                flowey_slot_id,
                enemy_sprite_path,
        ...
        if fire_present:
            ...
            fire_drawn = _battle_draw_story_slot(
                BATTLE_SLOT_STORY_FIRE,
                fire_path,
        ...
        if map1_story_toriel_visible:
            ...
            toriel_drawn = _battle_draw_story_slot(
                BATTLE_SLOT_STORY_TORIEL,
                toriel_path,
        ...
        enemy_drawn = _battle_draw_enemy_main_slot(enemy_x, enemy_y, enemy_sprite_w, enemy_sprite_h)
        if not enemy_drawn:
...
def _draw_map1_story_phase2_overlay(frame_x, frame_y, frame_w):
    ...
    if flowey_visible:
        drew_flowey = False
        flowey_slot_id = BATTLE_SLOT_FLOWEY_ANGRY if map1_story_enemy_angry else BATTLE_SLOT_FLOWEY_NORMAL
        drew_flowey = _battle_draw_story_slot(
            flowey_slot_id,
            enemy_sprite_path,
            MAP1_STORY_ENEMY_DRAW_W,
            MAP1_STORY_ENEMY_DRAW_H,
...
def _draw_png_in_box(png_info, x, y, w, h, preserve_aspect=True, allow_upscale=False):
    ...
    slot_kind = None
    if len(png_info) >= 4:
        slot_id, src_w, src_h, slot_kind = png_info[0], png_info[1], png_info[2], png_info[3]
    else:
    ...
    draw_x = x + ((w - draw_w) // 2)
    draw_y = y + ((h - draw_h) // 2)
    if slot_kind == BATTLE_SLOT_KIND_PNG:
        return battle_assets.draw_slot(slot_id, draw_x, draw_y, draw_w, draw_h)
    try:
...
def _draw_act_selection_indicator(prev_index, next_index, slots):
    ...
    if prev_index >= 0 and prev_index < len(slots):
        px, py, pw = _act_selection_line_rect(slots[prev_index])
        lgfx.draw_rect(px, py, pw, 1, 0x0000)
    if next_index >= 0 and next_index < len(slots):
        nx, ny, nw = _act_selection_line_rect(slots[next_index])
        lgfx.draw_rect(nx, ny, nw, 1, BATTLE_COLOR_RED)


def _act_selection_line_rect(slot):
    sx, sy, sw, sh = slot
    line_x = sx + 1
    line_y = sy + sh - 2
    line_w = sw - 2
...
def _act_slots_for_layout(mode, option_count, inner_x, inner_y, inner_w, inner_h):
    gap = 4
    if option_count <= 0 or inner_w < 12 or inner_h < 12:
        return None
    if option_count <= 2:
        slot_h = (inner_h - (gap * (option_count - 1))) // option_count
        if slot_h < 8:
            return None
        slots = []
        cursor_y = inner_y
        for _ in range(option_count):
            slots.append((inner_x, cursor_y, inner_w, slot_h))
            cursor_y += slot_h + gap
        return tuple(slots)
    if option_count == 4:
        ...
        slot1 = (inner_x + slot_w + gap, inner_y, slot_w, slot_h)
        slot2 = (inner_x, inner_y + slot_h + gap, slot_w, slot_h)
        slot3 = (inner_x + slot_w + gap, inner_y + slot_h + gap, slot_w, slot_h)
...
def _resolve_act_dialog_layout():
    option_count = _battle_enemy_act_option_count()
    if option_count < 1:
        option_count = 1
    frame_w = BATTLE_FRAME_W
    max_frame_w = ACTIVE_VIEW_W - 4
    ...
    while True:
        ...
        inner_h = dialog_h - 8
        if option_count <= 2:
            slots_col = _act_slots_for_layout(2, option_count, inner_x, inner_y, inner_w, inner_h)
            ...
            slots_grid = _act_slots_for_layout(1, option_count, inner_x, inner_y, inner_w, inner_h)
            if slots_grid and _act_slots_fit(slots_grid):
                return frame_w, frame_x, frame_y, cmd_x0, cmd_y, cmd_w, dialog_x, dialog_y, dialog_w, dialog_h, slots_grid
        else:
            slots_lr = _act_slots_for_layout(1, option_count, inner_x, inner_y, inner_w, inner_h)
        ...
        if frame_w >= max_frame_w:
            fallback_slots = None
            if option_count <= 2:
                fallback_slots = _act_slots_for_layout(2, option_count, inner_x, inner_y, inner_w, inner_h)
            elif option_count == 4:
                fallback_slots = _act_slots_for_layout(1, option_count, inner_x, inner_y, inner_w, inner_h)
            else:
                fallback_slots = _act_slots_for_layout(2, option_count, inner_x, inner_y, inner_w, inner_h)
...
def _battle_menu_opening_frame_w():
    return BATTLE_FRAME_W


def _draw_battle_menu_screen(dialog_active):
    global menu_frame_x_used, menu_frame_w_used, menu_cmd_y_used
    global act_menu_slot_cache, act_prev_selected_index, act_choice_index
    ...
    if act_menu_active:
        ...
        frame_w = _battle_menu_opening_frame_w()
        frame_x, frame_y, cmd_x0, cmd_y, cmd_w = _battle_menu_geometry(frame_w)
        dialog_x = frame_x + 10
        dialog_w = frame_w - 20
        dialog_h = 28
    ...
    if not act_menu_active and not item_menu_active:
        ...
        elif scripted_battle_active and scripted_battle_side_dialog:
            dialog_x, dialog_render_y, dialog_w, dialog_h = _scripted_battle_opening_dialog_rect(frame_x, frame_y, frame_w, cmd_y)
        else:
            if scripted_battle_active:
                dialog_h = _scripted_battle_dialog_png_size()[1]
                dialog_render_y = battle_menu_enemy_bottom_used + 10
                max_dialog_y = cmd_y - dialog_h - 2
            else:
    ...
    if act_menu_active and act_slots:
        if act_choice_index >= len(act_slots):
            act_choice_index = len(act_slots) - 1
        ...
        for i, slot in enumerate(act_slots):
            if i == act_choice_index:
                line_x, line_y, line_w = _act_selection_line_rect(slot)
                lgfx.draw_rect(line_x, line_y, line_w, 1, BATTLE_COLOR_RED)
            option_entry = _battle_enemy_act_option_entry(i)
            option_png_info = _battle_enemy_act_option_png_info(i)
...
def _battle_enemy_hp_max():
    profile = _battle_enemy_profile()
    max_hp = int(profile.get("max_hp", ENEMY_HP_MAX))
    if max_hp < 1:
        max_hp = ENEMY_HP_MAX
    return max_hp
...
def _battle_png_slot_info(slot_id, src_w, src_h):
    return (slot_id, src_w, src_h, BATTLE_SLOT_KIND_PNG)


def _battle_clear_dialog_image():
    global battle_dialog_png_info
    battle_assets.replace_dialog(None)
    battle_dialog_png_info = None


def _battle_set_dialog_image_from_path(path, src_w, src_h):
    global battle_dialog_png_info
    if not path:
        _battle_clear_dialog_image()
        return False
    if battle_assets.replace_dialog(path):
        battle_dialog_png_info = _battle_png_slot_info(BATTLE_SLOT_DIALOG_CURRENT, src_w, src_h)
        return True
    battle_dialog_png_info = None
    return False


def _battle_load_fixed_slot(slot_id, path, src_w, src_h):
    if not path:
        return None
    if not battle_assets.load_slot_once(slot_id, path):
        return None
    return _battle_png_slot_info(slot_id, src_w, src_h)


def _battle_set_fixed_dialog_slot(slot_id, path, src_w, src_h):
    global battle_dialog_png_info
    png_info = _battle_load_fixed_slot(slot_id, path, src_w, src_h)
    battle_assets.replace_dialog(None)
    if not png_info:
        battle_dialog_png_info = None
        return False
    battle_dialog_png_info = png_info
    return True


def _battle_enemy_act_option_count():
    profile = _battle_enemy_profile()
...
def _battle_enemy_act_option_slot_id(index):
    if index >= 0 and index < len(BATTLE_ACT_OPTION_SLOT_IDS):
        return BATTLE_ACT_OPTION_SLOT_IDS[index]
    return -1
...
def _battle_enemy_main_png_info():
    path, src_w, src_h = _battle_enemy_sprite_info()
    return _battle_load_fixed_slot(BATTLE_SLOT_ENEMY_MAIN, path, src_w, src_h)


def _battle_enemy_act_option_png_info(index):
    entry = _battle_enemy_act_option_entry(index)
    png_info = _enemy_entry_png_info(entry)
    if not png_info:
        return None
    slot_id = _battle_enemy_act_option_slot_id(index)
    if slot_id < 0:
        return None
    return _battle_load_fixed_slot(slot_id, png_info[0], png_info[1], png_info[2])


def _battle_enemy_mercy_png_info(slot_id, entry):
    png_info = _enemy_entry_png_info(entry)
    if not png_info:
        return None
    return _battle_load_fixed_slot(slot_id, png_info[0], png_info[1], png_info[2])
...
def _battle_apply_fixed_dialog_entry(slot_id, entry):
    global battle_dialog_text
    ...
    if not png_info:
        _battle_clear_dialog_image()
        return False
    return _battle_set_fixed_dialog_slot(slot_id, png_info[0], png_info[1], png_info[2])


def _battle_draw_enemy_main_slot(x, y, w, h):
    png_info = _battle_enemy_main_png_info()
    if not png_info:
        return False
    return battle_assets.draw_slot(BATTLE_SLOT_ENEMY_MAIN, x, y, w, h)


def _battle_draw_story_slot(slot_id, path, src_w, src_h, x, y, w, h):
    png_info = _battle_load_fixed_slot(slot_id, path, src_w, src_h)
    if not png_info:
        return False
    return battle_assets.draw_slot(slot_id, x, y, w, h)
...
def _battle_enemy_dialog_apply_entry(entry):
    ...
    png_info = _enemy_entry_png_info(entry)
    if png_info:
        _battle_set_dialog_image_from_path(png_info[0], png_info[1], png_info[2])
    else:
        _battle_clear_dialog_image()
...
def _battle_enemy_uses_scripted_battle():
    profile = _battle_enemy_profile()
    return profile.get("script_behavior") == "scripted_battle"


def _scripted_battle_dialog_line_ms():
    profile = _battle_enemy_profile()
...
def _scripted_battle_dialog_png_size():
    if scripted_battle_active and scripted_battle_index >= 0 and scripted_battle_index < len(scripted_battle_entries):
        png_info = _enemy_entry_png_info(scripted_battle_entries[scripted_battle_index])
        if png_info:
            return png_info[1], png_info[2]
    return SCRIPTED_BATTLE_DIALOG_PNG_W, SCRIPTED_BATTLE_DIALOG_PNG_H


def _scripted_battle_opening_dialog_rect(frame_x, frame_y, frame_w, cmd_y):
    profile = _battle_enemy_profile()
    status_y = cmd_y - (8 + BATTLE_STATUS_TO_CMD_GAP)
    status_band_top = status_y - BATTLE_HP_BAR_H - 3
    y_min = frame_y + 10
    ...
    enemy_mid_y = battle_menu_enemy_y + (battle_menu_enemy_h // 2)
    side_left = enemy_right + SCRIPTED_BATTLE_SIDE_DIALOG_GAP_PX
    side_right = frame_x + frame_w - 8
    if side_right <= side_left + 20:
        side_right = side_left + 20

    src_w, src_h = _scripted_battle_dialog_png_size()
    desired_w = int(profile.get("opening_dialog_desired_w", 164))
    if desired_w < 40:
        desired_w = 40
    dialog_w = desired_w
    avail_w = side_right - side_left
...
def _scripted_battle_apply_current_line(now_ms):
    global battle_dialog_mode, battle_dialog_started_ms, act_dialog_until_ms, battle_menu_dirty
    if scripted_battle_index < 0 or scripted_battle_index >= len(scripted_battle_entries):
        return
    _battle_enemy_dialog_apply_entry(scripted_battle_entries[scripted_battle_index])
    battle_dialog_mode = BATTLE_DIALOG_ACT_REPLY
    battle_dialog_started_ms = now_ms
    act_dialog_until_ms = time.ticks_add(now_ms, _scripted_battle_dialog_line_ms())
    battle_menu_dirty = True


def _scripted_battle_start_sequence(entries, kind, now_ms, side_dialog=False, return_to_act=False, pending_exit=False):
    global mercy_exit_pending
    global scripted_battle_active, scripted_battle_kind, scripted_battle_entries, scripted_battle_index
    global scripted_battle_next_ms, scripted_battle_side_dialog, scripted_battle_return_to_act, scripted_battle_pending_exit

    seq = tuple(entries) if entries else ()
    if not seq:
        return False
    mercy_exit_pending = False
    scripted_battle_active = True
    scripted_battle_kind = kind
    scripted_battle_entries = seq
    scripted_battle_index = 0
    scripted_battle_next_ms = 0
    scripted_battle_side_dialog = bool(side_dialog)
    scripted_battle_return_to_act = bool(return_to_act)
    scripted_battle_pending_exit = bool(pending_exit)
    _scripted_battle_apply_current_line(now_ms)
    scripted_battle_next_ms = act_dialog_until_ms
    return True


def _battle_enter_attack_mode(loop_start):
    global mode, battle_menu_dirty, battle_dialog_visible
    global battle_dialog_mode, battle_dialog_text, act_dialog_until_ms, mercy_exit_pending
...
def _battle_enter_fight_mode(loop_start):
    global mode, fight_return_deadline_ms
...
def _scripted_battle_finish():
    ...
    pending_exit = scripted_battle_pending_exit
    return_to_act = scripted_battle_return_to_act
    finished_kind = scripted_battle_kind
    finished_enemy_id = _battle_enemy_id()
    opening_enter_attack = bool(_battle_enemy_profile().get("opening_enter_attack"))
    ...
    if pending_exit:
        ...
        return
    if finished_kind == SCRIPTED_BATTLE_KIND_OPENING and opening_enter_attack:
        _battle_enter_attack_mode(time.ticks_ms())
        return
    if return_to_act:
...
def _scripted_battle_begin_opening(now_ms):
    profile = _battle_enemy_profile()
    opening = profile.get("opening_dialog", ())
    if not opening:
        return False
    return _scripted_battle_start_sequence(
        opening,
        SCRIPTED_BATTLE_KIND_OPENING,
        now_ms,
        side_dialog=bool(profile.get("opening_side_dialog")),
    )


def _scripted_battle_update(loop_start):
    global scripted_battle_index, scripted_battle_next_ms, scripted_battle_opening_pending

    if (not _battle_enemy_uses_scripted_battle()) or _map1_story_is_active():
        return False
    if scripted_battle_opening_pending:
        if time.ticks_diff(loop_start, scripted_battle_opening_due_ms) >= 0:
            scripted_battle_opening_pending = False
            _scripted_battle_begin_opening(loop_start)
        return True
    if not scripted_battle_active:
        return False
    if time.ticks_diff(scripted_battle_next_ms, loop_start) > 0:
        return True
    next_index = scripted_battle_index + 1
    if next_index >= len(scripted_battle_entries):
        _scripted_battle_finish()
        return True
    scripted_battle_index = next_index
    _scripted_battle_apply_current_line(loop_start)
    scripted_battle_next_ms = act_dialog_until_ms
    return True


def _scripted_battle_unlock_target_step():
    profile = _battle_enemy_profile()
    order = profile.get("act_unlock_order")
    if not order:
        return 0
    return len(order)


def _scripted_battle_apply_act_progress(selected):
    global act_sequence_step
    ...
    if selected == order[0]:
        act_sequence_step = 1
        return
    if act_sequence_step > 0 and act_sequence_step < len(order) and selected == order[act_sequence_step]:
        act_sequence_step += 1
        return
    act_sequence_step = 0


def _scripted_battle_start_act_sequence(selected, loop_start):
    global act_menu_active, act_nav_prev_dir, act_menu_slot_cache, act_prev_selected_index, act_selection_dirty
    ...
    if selected < 0 or selected >= len(sequences):
        return False
    _scripted_battle_apply_act_progress(selected)
    act_menu_active = False
    act_nav_prev_dir = 0
    act_menu_slot_cache = None
    act_prev_selected_index = -1
...
def _scripted_battle_try_mercy(loop_start):
    global battle_dialog_mode, battle_dialog_started_ms, act_dialog_until_ms, mercy_exit_pending, battle_menu_dirty

    target_step = _scripted_battle_unlock_target_step()
    if target_step > 0 and act_sequence_step == target_step:
        profile = _battle_enemy_profile()
        mercy_seq = profile.get("mercy_success_sequence", ())
        if mercy_seq and _scripted_battle_start_sequence(
            mercy_seq,
            SCRIPTED_BATTLE_KIND_MERCY,
            loop_start,
            side_dialog=False,
            pending_exit=True,
        ):
    ...
    _battle_apply_fixed_dialog_entry(BATTLE_SLOT_MERCY_LOCKED, _battle_enemy_mercy_locked_entry())
...
def _map1_story_show_line(index, now_ms):
    ...
    battle_dialog_started_ms = now_ms
    png_info = _map1_story_dialog_png_info(index)
    if png_info:
        _battle_set_dialog_image_from_path(png_info[0], png_info[1], png_info[2])
    else:
        _battle_clear_dialog_image()
...
def _enter_death_screen(now_ms):
    global mode, player_hp, mercy_exit_pending, map_boss_battle_enemy_id, map6_boss_battle_active
...
def _return_to_title_from_death():
    ...
    # Switch modes before native cleanup; bad PSRAM state should not trap the death screen.
    if hasattr(lgfx, "set_brightness"):
        lgfx.set_brightness(255)
    player_hp = PLAYER_HP_MAX
    mode = MODE_TITLE_MENU
...
def _exit_battle_to_explore():
    global mode, encounter_cooldown_frames, mercy_exit_pending, map_boss_battle_enemy_id, map6_boss_battle_active
    ...
    battle_assets.cleanup()
    print("[battle_assets] cleanup_after mem_free=", gc.mem_free())
...
def _start_battle_from_explore(enemy_id=None):
    ...
    explore_overlay_dirty = False
    _clear_interact_hints()
    boss = _map_boss_for_enemy_id(enemy_id)
    map_boss_battle_enemy_id = boss.get("enemy_id") if boss else None
    ...
    _reset_battle_state()
    if boss:
        _ensure_map_boss_sheet_loaded(boss)
    if current_map_id == MAP1_ID and (not map1_opening_battle_done):
        _map1_story_begin(time.ticks_ms())
    else:
        _map1_story_reset()
    if _battle_enemy_uses_scripted_battle():
        profile = _battle_enemy_profile()
        delay_ms = int(profile.get("opening_delay_ms", SCRIPTED_BATTLE_DIALOG_DELAY_MS))
        if delay_ms < 0:
            delay_ms = 0
        scripted_battle_opening_pending = True
        scripted_battle_opening_due_ms = time.ticks_add(time.ticks_ms(), delay_ms)
...
def _draw_battle_status_line(in_menu=False):
    if not hasattr(lgfx, "draw_text"):
        return
    enemy_hp_max = _battle_enemy_hp_max()
...
def draw_battle_attack_screen(full_refresh=False):
    ...
    if full_refresh:
        ...
        enemy_drawn = _battle_draw_enemy_main_slot(
            enemy_x,
            enemy_y,
            ATTACK_ENEMY_DRAW_W,
            ATTACK_ENEMY_DRAW_H,
...
def update_battle_menu(loop_start, fight_pressed, act_pressed, item_pressed, mercy_pressed):
    ...
    if _scripted_battle_update(loop_start):
        return
    ...
    if item_menu_active:
        if fight_pressed:
            _battle_enter_attack_mode(loop_start)
            return
        if act_pressed:
            _reset_item_menu_state()
            act_menu_active = True
        ...
        if mercy_pressed:
            _reset_item_menu_state()
            if _battle_enemy_uses_scripted_battle():
                _scripted_battle_try_mercy(loop_start)
            else:
                if act_sequence_step == 3:
                    print("MERCY: success")
                    battle_dialog_mode = BATTLE_DIALOG_MERCY_EXIT
                    _battle_apply_fixed_dialog_entry(BATTLE_SLOT_MERCY_SUCCESS, _battle_enemy_mercy_success_entry())
    ...
    if act_menu_active:
        ...
        if option_count < 1:
            option_count = 1
        if act_choice_index >= option_count:
            act_choice_index = option_count - 1
        if nav_dir != 0 and nav_dir != act_nav_prev_dir:
            prev_choice = act_choice_index
            if nav_dir > 0:
                act_choice_index = (act_choice_index + 1) % option_count
            else:
                act_choice_index = (act_choice_index + option_count - 1) % option_count
        ...
        if act_pressed:
            ...
            if _battle_enemy_uses_scripted_battle():
                _scripted_battle_start_act_sequence(selected, loop_start)
            else:
                reply_index = 0
                if selected == 0:
                    act_sequence_step = 1
...
def update_battle_attack(loop_start, fight_pressed):
    ...
    _battle_enter_fight_mode(loop_start)
...
def draw_all(loop_start):
    ...
    if mode == MODE_EXPLORE:
        wood_up_dialog_ended = _update_wood_up_dialog_timeout(loop_start)
        scene_redrawn = False
        ...
        lamp_dialog_active = time.ticks_diff(lamp_dialog_until_ms, loop_start) > 0
        wood_up_dialog_active = _wood_up_dialog_is_active(loop_start)
        dialog_active = lamp_dialog_active or wood_up_dialog_active
        dialog_needs_redraw = False
        if dialog_active:
            if wood_up_dialog_ended:
                dialog_needs_redraw = False
            elif not explore_overlay_dirty:
                dialog_needs_redraw = True
            elif scene_redrawn:
                dialog_needs_redraw = True
            elif player_redrawn:
                if lamp_dialog_active:
                    dialog_x, dialog_y, dialog_w, dialog_h = _lamp_dialog_rect()
                else:
                    dialog_x, dialog_y, dialog_w, dialog_h = _wood_up_dialog_rect()
                if player_sheet_enabled:
        ...
        if dialog_needs_redraw:
            if lamp_dialog_active and _draw_explore_lamp_dialog(loop_start):
                explore_overlay_dirty = True
            elif wood_up_dialog_active and _draw_wood_up_dialog(loop_start):
                explore_overlay_dirty = True
        ...
        _draw_map_boss(loop_start, scene_redrawn, player_redrawn)
    ...
    if mode == MODE_BATTLE_MENU:
        ...
        if battle_menu_dirty or dialog_active != battle_dialog_visible:
            ...
            act_prev_selected_index = act_choice_index
            act_selection_dirty = False
        if (not dialog_active) or _map1_story_is_active():
...
def _run_main_loop():
    ...
    global wood_up_dialog_until_ms
    global explore_overlay_dirty, map_boss_battle_enemy_id, map6_boss_battle_active
    while True:
            ...
            if portal_transition_active:
                ...
                if mode == MODE_EXPLORE:
                    ...
                    if mode == MODE_EXPLORE and encounter_cooldown_frames == 0 and teleport_cooldown_frames == 0:
                        boss_enemy_id = _map_boss_trigger_enemy_id(player_x, player_y)
                        if boss_enemy_id:
                            map_boss_battle_enemy_id = boss_enemy_id
                            map6_boss_battle_active = boss_enemy_id == "MAP6_BOSS"
                            _start_battle_from_explore(enemy_id=boss_enemy_id)
