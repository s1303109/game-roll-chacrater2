...
SRC_COLLISION_PATH = Path("/workspace/maps 11-1collision.jpeg")
OUT_DIR = Path("/workspace/row/assets/out_map11_1")
...
def prepare_collision_image(src):
    ...
    w, h = img.size
    if w != TARGET_W:
        raise SystemExit("collision width must be 640")
    if h != TARGET_H:
        raise SystemExit("collision height must be 960")
    return img
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Build Map11-1 collision from red mask image (16x16, red>=40% => blocked)."
    )
