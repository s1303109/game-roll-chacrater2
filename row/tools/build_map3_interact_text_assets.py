...
ASSETS = (
    ("map3_book_dialog_01.png", "一本古老的書"),
    ("map3_book_dialog_02.png", "書頁上寫著看不懂的符號"),
    ("map3_book_dialog_03.png", "但不知道為什麼"),
    ("map3_book_dialog_04.png", "你覺得它正在念你的名字"),
)
...
def main() -> None:
    ...
    scratch = Image.new("L", (DEFAULT_CANVAS_W * 3, CANVAS_H * 4), 0)
    draw = ImageDraw.Draw(scratch)
    canvas_w = DEFAULT_CANVAS_W
    for _, text in ASSETS:
        metrics = char_metrics(draw, text, font)
        req_w = required_canvas_width(metrics, TEXT_TARGET_H, TEXT_PAD_X)
        if req_w > canvas_w:
            canvas_w = req_w
    for filename, text in ASSETS:
        render_dialogue_asset(filename, text, font, canvas_w)
